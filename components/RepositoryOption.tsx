...
import { Repository } from './Example'
...
export function RepositoryOption(props: Props) {
  return (
    <Combobox.Option
      ...
      onClick={() => handleOnClick(props.repository.html_url)}
    >
      {({ active }) => (
        <>
          <header className="flex items-center">
            <CpuChipIcon
              ...
              aria-hidden="true"
            />
            <span className="ml-1 font-bold flex-auto truncate">{props.repository.name}</span>
          </header>
          ...
          <footer className="flex items-center justify-between">
            <div className="flex items-center space-x-2">
              ...
              <span className="flex items-center space-x-1">
                ...
                <span className="font-medium">{props.repository.owner.login}</span>
              </span>
              {/* Language */}
              <span className="flex items-center space-x-1">
                <span className="block w-1.5 h-1.5 rounded-full bg-indigo-400" />
                <span className="font-medium">{props.repository.language}</span>
              </span>
            </div>
            <div className="flex items-center space-x-2.5">
              ...
              <span className="flex items-center space-x-0.5">
                ...
                <span>{props.repository.stargazers_count} stars</span>
              </span>
              {/* Issues */}
              <span className="flex items-center space-x-0.5">
                <ViewfinderCircleIcon className="w-4 h-4" />
                <span>{props.repository.open_issues_count} issues</span>
              </span>
              {/* Forks */}
              <span className="flex items-center space-x-0.5">
                <svg
                ...
                <span>{props.repository.forks_count} forks</span>
              </span>
            </div>
          </footer>
        </>
